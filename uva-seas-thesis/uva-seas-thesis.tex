...
\title{\thispackage\\{\small (version 1.1.15)}}
\author{
  Joel Coffman\\
...
\Gls{UVa} Print Services~\cite{uvaprint2001thesis} specifies that the abstract must be 350 words or less;
...
\begin{longtable}{r@{.}r@{.}rp{.8\linewidth}}
  %\toprule
...
\paragraph{Font size}
\Gls{SEAS}~\cite{seas2008instructions} mandates a standard typeface with 10 or 12 characters per inch.
...
The suggested page order~\cite{uvaprint2001thesis, seas2008instructions} is as follows:
\begin{itemize*}
  \item Thesis
  \begin{itemize*}
    \item Title page
...
Please be aware that \gls{SEAS}~\cite{seas2008instructions} is actually inconsistent regarding the page order.
...
In addition, \gls{SEAS}~\cite{seas2008instructions} mandates that a list of symbols follow the list of figures.
...
Page numbers should appear in the upper-right corner of pages~\cite{uvaprint2001thesis, seas2008instructions}.
...
\Gls{SEAS}~\cite{seas2008instructions} requires page numbers to appear in the page margin and all other text to be kept within the page's margins.
...
Please note that this package takes minor liberties with regards to the example title and signatures pages provided by \Gls{SEAS}~\cite{seas2008instructions}.
Although the example is not reproduced perfectly, the deviations are minor.

\paragraph{Abstract}
Both \Gls{UVa} Print Services~\cite{uvaprint2001thesis} and \gls{SEAS}~\cite{seas2008instructions} specify that the abstract must be 350 words or less.\footnote{
...
\paragraph{Figures}
\Gls{SEAS}~\cite{seas2008instructions} requires each figure to follow its text reference as closely as possible.
